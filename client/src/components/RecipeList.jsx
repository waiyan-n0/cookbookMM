...
const RecipeList = ({ type, id }) => {
    ...
    useEffect(() => {
        ...
        fetch(`http://localhost:3000/recipes/${id}/recipes_list`)
            ...
            .catch(error => {
                console.log('Error Fetching Your Recipes List:', error);
                setNoti({ show: true, msg: "Error fetching your recipes list!", type: "error" });
            });
    }, [id]);

    const editRecipe = (e, id) => {
        e.stopPropagation();
        const recipeToEdit = recipesList.find((recipe) => recipe._id === id);
        if (recipeToEdit) {
    ...
    const deleteRecipe = async (e, id) => {
        ...
        try {
            ...
            if (!response.ok) {
                setNoti({show:true, msg: `${deletedRecipe.msg || 'Failed to delete recipe'}`, type: "error"});
            } else {
                setNoti({show:true, msg: `${deletedRecipe.msg || 'Deleted successfully!'}`, type: "success"});
                setRecipesList(prev => prev.filter(recipe => recipe._id !== id));
            }
        } catch (err) {
            console.log('Error Deleting Recipe!', err);
            setNoti({show:true, msg: `Error Deleting Recipe!`, type: "error"});
        }
    }
    ...
    return (
        <section className='flex flex-col w-full'>
            ...
            {recipesList.length === 0 ? (
                <div className="text-center p-12 border-2 border-dashed border-slate-200 rounded-2xl text-slate-400 bg-slate-50/50 m-4">
                    You haven't created any recipes yet! <br/> Open Side Bar to create a new recipe.
                </div>
            ) : (
                <div className='flex flex-row flex-wrap gap-6 p-4 justify-start'>
                    {recipesList.map((recipe) => (
                        <div key={recipe._id} onClick={() => navigate(`/recipes/${recipe._id}`)}
                            ...
                            <div className="relative aspect-[4/3] w-full overflow-hidden bg-gray-100">
                                <button onClick={(e) => editRecipe(e, recipe._id)}
                                        className='absolute flex items-center justify-center right-2 top-2 text-black cursor-pointer bg-white p-1 rounded-md shadow-sm z-10 hover:scale-105 transition-transform'>
                                ...
                                <button onClick={(e) => deleteRecipe(e, recipe._id)}
                                        className='absolute flex items-center justify-center right-10 top-2 text-red-500 cursor-pointer bg-white p-1 rounded-md shadow-sm z-10 hover:scale-110 transition-transform'>
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4">
                            ...
                            <div className="p-4">
                                ...
                                <div className="flex items-center justify-around text-sm text-gray-600 pt-1">
                                    <button onClick={(e) => e.stopPropagation()}
                                            className="flex items-center gap-1.5 text-gray-600 transition-colors py-1 px-2 rounded-md hover:bg-gray-50">
            ...
            <CreateRecipe key={selectedRecipe?._id || "createRecipe"} isOpen={isUpdateModalOpen}
                          onClose={() => {
                              setIsUpdateModalOpen(false);
                              setSelectedRecipe(null);
                          }}
                          selectedRecipe={selectedRecipe}
                          author={selectedRecipe?.author ? { _id: selectedRecipe.author.id, name: selectedRecipe.author.name } : null}
            />
        </section>
    );
}
